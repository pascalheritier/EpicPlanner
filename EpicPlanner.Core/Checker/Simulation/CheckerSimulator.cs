    ...
    private void WriteEpicWorksheet(ExcelPackage _Package)
    {
                     ...
                     .OrderBy(s => s.Epic, StringComparer.OrdinalIgnoreCase))
        {
            ...
            string reliabilityFormula =
                $"=IF(ISBLANK(B{epicRow}),\"\",IF(C{epicRow}<=0,IF(D{epicRow}>0,0,0),MIN(1,MAX(0,1-ABS(F{epicRow}-E{epicRow})/C{epicRow}))))";
            string usageFormula =
                $"=IF(C{epicRow}<=0,IF(D{epicRow}>0,2,0),D{epicRow}/C{epicRow})";
